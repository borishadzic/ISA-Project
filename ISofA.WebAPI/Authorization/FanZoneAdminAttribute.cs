...
namespace ISofA.WebAPI.Authorization
{
    ...
    public class FanZoneAdminAttribute : AuthorizationFilterAttribute
    {
        private IAuthService _authService { get { return (IAuthService)GlobalConfiguration.Configuration.DependencyResolver.GetService(typeof(IAuthService)); } }
        ...
        protected virtual bool IsAuthorized(HttpActionContext actionContext)
        {
            ...
            int theaterId = Convert.ToInt32(actionContext.ControllerContext.RouteData.Values["id"]);
            string userId = user.FindFirst(ClaimTypes.NameIdentifier).Value;

            if (!_authService.IsFanZoneAdmin(userId, theaterId))
                return false;            
